...
def predict(img_path, models):
    ...
    # Get both the predicted class and its confidence score
    predicted_class = np.argmax(result[0])
    confidence_score = result[0][predicted_class] * 100  # Convert to percentage
...
def upload():
    ...
    if request.method == 'POST':
        ...
        if file:
            ...
            classes, accuracy = predict(filepath, models)
            session['classes'] = str(classes)
            session['accuracy'] = str(accuracy)
...
def hasil():
    ...
    accuracy = float(session.get('accuracy'))
